      ...
      try {
        ...
        const activationPath =
          'sso_user_redirect_url' in res && res.sso_user_redirect_url
            ? getSsoActivationPath(res.sso_user_redirect_url as string, provider)
            : null;
        ...
        if ('enable_mfa' in res && res.enable_mfa) {
          const mfaRes = res as unknown as { mfaId: string; mfaType: number };
          return navigate(`/verify-mfa?mfa_id=${mfaRes.mfaId}&mfa_type=${mfaRes.mfaType}`);
        }
        navigate('/login', { replace: true });
      } catch (error: any) {
